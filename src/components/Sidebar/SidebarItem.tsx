            ...
            <div
                ...
                className={`flex items-center justify-between h-[40px] px-[23px] rounded cursor-pointer transition-colors duration-150
                    ${!collapsed && isActive
                        ...
                        : 'text-black hover:bg-[#E6F7FF] hover:text-[#1890FF]'}
                `}
            >
                {item.path ? (
                    <NavLink
                ...
                <div className="ml-2 mt-1 space-y-[10px]">
                    {item.children!.map((sub, idx) => {
                        ...
                        return sub.disabled ? (
                            <div
                                key={idx}
                                className="block h-[40px] px-[23px] text-sm font-normal rounded flex items-center text-gray-400 cursor-not-allowed"
                            >
                                {sub.label}
                            </div>
                        ) : (
