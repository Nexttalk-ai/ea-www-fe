...
const EditTSPID: React.FC = () => {
    ...
    useEffect(() => {
        const fetchTSPID = async () => {
            ...
            try {
                ...
                setConfigName(tspid.tspid_value);
                
                // Extract entries from content
                const entries: TSPIDEntry[] = [];
                if (tspid.content && typeof tspid.content === 'object') {
                    Object.keys(tspid.content).forEach(tspidKey => {
                        const config = tspid.content[tspidKey];
                        entries.push({
                            id: uuidv4(),
                            tspid_value: tspidKey,
                            pixel_id: config.pixel_id || '',
                            source: config.source || 'facebook'
                        });
                    });
                }
                
                setTspidEntries(entries);
            } catch (err) {
    ...
    const handleSave = async (dataToSave?: any) => {
        ...
        try {
            ...
            // Update the TSPID config using the service
            ...
            console.error('Error updating TSPID config:', err);
            setError(err instanceof Error ? err.message : 'Failed to update TSPID configuration');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <HomeLayout>
            <div className="container mx-auto px-4 py-8">
                <div className="flex items-center gap-4 mb-6">
                    ...
                    <h1 className="text-2xl font-bold">Edit TSPID Configuration</h1>
                </div>
